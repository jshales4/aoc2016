...
def main():
    ...
    while threshold_reached == False:
        result = climb_tree(start, move_tracker,0)
        move_tracker = result[0]
        threshold_reached = result[1]
    for n in move_tracker:
        if move_tracker[n]<51:
            dict_size +=1
    print "There are this many unique spaces: ", dict_size
    print datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def climb_tree(position, move_tracker, counter):
    if counter ==51:
        return move_tracker, True
    elif len(position.move_options)==0:
        attempt = make_moves(position,move_tracker)
        move_tracker = attempt[0]
        return move_tracker, False
    else:
        for n in position.move_options:
                attempt = climb_tree(n,move_tracker, counter + 1)
                move_tracker = attempt[0]
                if attempt[1]==True:
                    break
...
def make_moves(position, move_tracker):
    for n in [-1,1]:
            if is_wall(position.xpos, position.ypos+n) == False:
                ...
                if str(position.xpos)+ '_' + str(position.ypos+n) not in move_tracker or move_tracker[str(position.xpos)+ '_' + str(position.ypos+n)]>position.moves_made+1:
                    move_tracker[str(position.xpos)+ '_' + str(position.ypos+n)] = position.moves_made + 1
            ...
            if is_wall(position.xpos+n, position.ypos) == False:
                ...
                #     print position.moves_made +1
                #     return move_tracker, position.moves_made + 1
                if str(position.xpos+n)+ '_' + str(position.ypos) not in move_tracker or move_tracker[str(position.xpos+n)+ '_' + str(position.ypos)]>position.moves_made+1:
                    move_tracker[str(position.xpos+n)+ '_' + str(position.ypos)] = position.moves_made + 1
